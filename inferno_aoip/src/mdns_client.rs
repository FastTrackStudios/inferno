...
impl MdnsClient {
                                ...
                                if let Some(txt) = data.as_txt() {
                                    if let Some(data) = txt.txt_data().get(0) {
                                        if data.eq_ignore_ascii_case(&self.self_origin) {
                                            ...
                                            debug!(
                                                "got local answer from other instance, considering"
                                            );
                                            return true;
                                        }
                                    }
    ...
    pub async fn a_record_exists(&self, fqdn_parts: &[&str]) -> Result<bool, Box<dyn Error>> {
        ...
        for _ in 0..3 {
            let r = self
                .a_record_exists_single_check(fqdn_parts, Duration::from_millis(400))
                .await?;
            if r {
                return Ok(r);
            };
    ...
    pub async fn is_multicast_ip_already_used(
        &self,
        addr: Ipv4Addr,
    ) -> Result<bool, Box<dyn Error>> {
    ...
    pub async fn query(&self, fqdn_parts: &[&str]) -> Result<AdvertisedService, Box<dyn Error>> {
        ...
        if let Some((name, port)) = target {
            ...
            let a_response = self
                .do_single_query(&[RecordType::A], &name, Duration::from_secs(3))
                .await?;
