...
impl<T, P: ProxyToBuffer<Atomic<T>>> RingBufferShared<T, P> {
    ...
    fn commit_readable_pos(&self) {
        if let Some(dest) = &self.readable_pos_dest {
            dest.tab[dest.offset]
                .store(self.readable_pos.load(Ordering::Relaxed), Ordering::Release);
        }
    }
...
impl<T: Default + NoUninit, P: ProxyToBuffer<Atomic<T>>> RBInput<T, P> {
    pub fn new(shared: Arc<RingBufferShared<T, P>>, hole_fix_wait: usize) -> Self {
        ...
        Self {
            rb: shared,
            item_ready: boolvec![false; items_size],
            hole_fix_wait,
        }
    }
    ...
    pub fn write_from_at(
        ...
        hole |= self.rb.readable_pos.load(Ordering::Relaxed)
            != self.rb.writing_pos.load(Ordering::Relaxed);
        ...
        if hole {
            self.close_items_until_internal(
                self.rb
                    ...
                    .wrapping_sub(self.hole_fix_wait),
                self.rb.writing_pos.load(Ordering::Relaxed),
            );
        }
    ...
    fn close_items_until_internal(&self, close_until_pos: usize, check_until_pos: usize) {
        ...
        ) > 0
        {
            ...
            self.rb
                .readable_pos
                .store(close_until_pos, Ordering::Release);
        }
        ...
        ) > 0
        {
            ...
            self.rb
                .readable_pos
                .store(new_readable_pos, Ordering::Release);
        }
    }
    ...
    pub fn close_items_until(&self, mut until_pos: usize) {
        ...
        if need_until_pos != until_pos {
            ...
            self.rb
                .readable_pos
                .store(need_until_pos, Ordering::Release);
        }
            ...
            .map(|buffer| {
                ...
                for_in_ring(
                    ...
                    |rb_index| {
                        ...
                        output[out_index] =
                            buffer[rb_index * self.rb.stride].load(Ordering::Relaxed);
                        out_index += 1;
                    },
                );
            })
...
pub fn new_owned<T: Default>(
    ...
    RBInput<T, OwnedBuffer<Atomic<T>>>,
    RBOutput<T, OwnedBuffer<Atomic<T>>>,
) {
    let shared = RingBufferShared::new(OwnedBuffer::<Atomic<T>>::new(length), 1, start_time, None);
    (
        RBInput {
            rb: shared.clone(),
            item_ready: boolvec![false; shared.items_size],
            hole_fix_wait,
        },
        RBOutput { rb: shared },
    )
...
impl<T> ExternalRBInput<T> {
    fn advance(&self, new_position: usize) {
        ...
        self.rb
            .readable_pos
            .store(new_position.wrapping_sub(self.margin), Ordering::Release);
        self.rb.commit_readable_pos();
    }
...
pub fn shared_from_external<T: Default>(
    ...
    let external =
        unsafe { ExternalBuffer::<Atomic<T>>::new(par.ptr, par.length, par.valid.clone()) };
    RingBufferShared::new(
        external,
        par.stride,
        start_time,
        par.readable_pos_dest.clone(),
    )
}
...
pub fn wrap_external_source<T: Default>(
    ...
    RBOutput {
        rb: shared_from_external(par, start_time),
    }
}
