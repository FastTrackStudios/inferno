...
use crate::net_utils::{MAX_PAYLOAD_BYTES, create_mio_udp_socket};
...
use crate::{
    mdns_client::MdnsClient, protocol::flows_control::FlowsControlClient,
    protocol::mcast::MulticastMessage,
};

use crate::ring_buffer::{
    self, ExternalBuffer, ExternalBufferParameters, OwnedBuffer, ProxyToSamplesBuffer, RBInput,
    RBOutput, RingBufferShared,
};
...
impl ChannelsBuffering<OwnedBuffer<Atomic<Sample>>> for OwnedBuffering {
    fn connect_channel(
        ...
        let (sink, source) = if rb_output.is_none() {
            let (sink, source) = ring_buffer::new_owned::<Sample>(
                self.buffer_length,
                start_time as usize,
                self.hole_fix_wait,
            );
        ...
        tokio::spawn(async move {
            sc.connect_channel(channel_index, source, latency_samples)
                .await;
        });
        sink
    }
...
impl ExternalBuffering {
    pub fn new(channels: Vec<ExternalBufferParameters<Sample>>, hole_fix_wait: usize) -> Self {
        ...
        let ring_buffers = (0..channels_count)
            .map(|chi| ring_buffer::shared_from_external(&channels[chi], 0))
            .collect_vec();
        Self {
            channels,
            hole_fix_wait,
...
impl ChannelsBuffering<ExternalBuffer<Atomic<Sample>>> for ExternalBuffering {
    ...
    fn disconnect_channel(
        ...
        tokio::spawn(async move {
            flows_recv
                .disconnect_channel(flow_index, channel_in_flow, channel_index, rb_shared)
                .await;
        });
    }
}
    ...
    ChannelsSubscriberInternal<P, B>
{
    ...
    pub async fn resolve_subscriptions(&mut self) -> bool {
            ...
            needed_channel_indices.retain(|&ch_index| {
                ...
                match alias_map.entry((ch.tx_hostname.clone(), ch.tx_channel_name.clone())) {
                    Entry::Occupied(existing_index) => {
                        channel_index_aliases
                            .entry(*existing_index.get())
                            .or_default()
                            .push(ch_index);
                        return false;
                    }
        ...
        let dns_resolve_futures =
            needed_channel_indices
                .iter()
                .enumerate()
                .map(|(i, &local_ch_index)| {
                    ...
                    async move {
                        ...
                        let result = mdns_client
                            .query_chan(&chsub.tx_hostname, &chsub.tx_channel_name)
                            .await;
                        debug!("{:?}", result);
                        return (
                            local_ch_index,
        ...
        let mut channels_per_server =
            BTreeMap::<SocketAddr, BTreeMap<usize, AdvertisedChannel>>::new();
            ...
            if let Some(advserv) = advserv_opt {
                match &advserv.multicast {
                    ...
                    Some(mcast) => {
                        ...
                        info!(
                            "using multicast for channel {} of {}",
                            advserv.tx_channel_id, mcast.tx_hostname
                        );
                        channels_per_bundle
                            .entry(bundle_name)
                            .or_default()
        ...
        let bad_servers = channels_per_server
            ...
            .filter(|(_addr, entries)| {
                entries
                    .iter()
                    .map(|(_, advch)| advch.tx_channels_per_flow)
                    .unique()
                    .count()
                    != 1
                    || entries
                        .iter()
                        .map(|(_, advch)| advch.bits_per_sample)
                        .unique()
                        .count()
                        != 1
                    || entries
                        .iter()
                        .map(|(_, advch)| advch.dbcp1)
                        .unique()
                        .count()
                        != 1
            })
        ...
        let mut bundles: BTreeMap<_, _> = join_all(dns_resolve_futures)
            .await
            .into_iter()
            .flatten()
            .collect();

        let mut free_slots_per_server =
            BTreeMap::<SocketAddr, BTreeMap<usize, BTreeSet<usize>>>::new();
        let mut futures_per_server = BTreeMap::<SocketAddr, Vec<_>>::new();
        {
                                ...
                                Entry::Occupied(mut channels) => {
                                    ...
                                    let mut updates = channels
                                        ...
                                        .filter_map(|(lci, advch)| {
                                            match flow.tx_channels.iter().enumerate().position(
                                                |(_, &txch)| txch == Some(advch.tx_channel_id),
                                            ) {
                                                None => None,
                                                Some(channel_in_flow) => {
                                                    Some(ChannelSourceUpdate {
                                                        // here we've found a channel that we need
                                                        local_channel_indices:
                                                            channel_index_aliases[lci].clone(),
                                                        remote: ChannelOtherEnd {
                                    ...
                                    let free_slots = free_slots_per_server
                                        ...
                                        .entry(flow_index)
                                        .or_default();
                                    for (channel_in_flow, &refcount) in
                                        flow.channels_refcount.iter().enumerate()
                                    {
                                        if refcount <= 0 {
                                    ...
                                    for update in &updates {
                                        ...
                                        let result = channels
                                            .get_mut()
                                            .remove(&update.local_channel_indices[0]);
                                ...
                                Entry::Occupied(channels) => {
                                    ...
                                    let mut updates = channels
                                        ...
                                        .map(|(lci, advch)| ChannelSourceUpdate {
                                            local_channel_indices: channel_index_aliases[lci]
                                                .clone(),
                                            remote: ChannelOtherEnd {
                                                local_flow_index: flow_index,
                                                channel_in_flow: advch
                                                    .multicast
                                                    .as_ref()
                                                    .unwrap()
                                                    .channel_in_bundle,
            ...
            for (&server_addr, channels) in &mut channels_per_server {
              ...
              return true;
                        ...
                        let futures = tx_ch_updates_per_flow.into_iter().map(
                            |(flow_index, (new_tx_channels, updates))| {
                                ...
                                if let FlowSource::Unicast(uf) = &flow.source {
                                    ...
                                    async move {
                                            ...
                                            .await
                                        {
                                            ...
                                            Err(e) => {
                                                error!(
                                                    "update flow {updates:?} request failed: {e:?}"
                                                );
            ...
            for (bundle_full_name, advbundle) in &bundles {
                let socket = match mio::net::UdpSocket::bind(advbundle.media_addr) {
                    ...
                    Err(e) => {
                        error!(
                            "failed to bind to socket {:?} for multicast media: {e:?}",
                            advbundle.media_addr
                        );
                        continue;
                    }
                };
                            ...
                            Err(e) => {
                                ...
                                for update in updates {
                                    for chi in update.local_channel_indices {
                                        subinfos[chi].as_mut().unwrap().status =
                                            SubscriptionStatus::TxFail;
                                    }
                                }
                                flows.lock().unwrap()[flow_index] = None;
                                return None;
        ...
        while let Some(updates) = rx.recv().await {
            ...
            let now = self
                ...
                .write()
                .unwrap()
                .wrapping_now_in_timebase(self.self_info.sample_rate as u64);
            let now = match now {
                Some(v) => v,
                None => {
            ...
            for upd in updates {
                ...
                info!(
                    ...
                    self.self_info.rx_channels[first_index]
                        .friendly_name
                        .read()
                        .unwrap(),
    ...
    async fn scan_flows(&mut self) -> bool {
        ...
        let mut channels_changed = vec![];
        {
            ...
            let min_flow_creation_time =
                self.ref_instant.elapsed().as_secs().saturating_sub(6) as _;
        ...
        join_all(
            destroy_futures_per_remote
                .iter_mut()
                .map(|(_, ops)| async move {
                    for op in ops {
                        op.await;
                    }
                }),
    ...
    async fn handle_command(&mut self, command: Command) -> bool {
        match command {
            ...
            Command::Subscribe {
                ...
                self.subscribe(local_channel_index, &tx_channel_name, &tx_hostname)
                    .await;
            }
            Command::Unsubscribe {
                local_channel_index,
    ...
    pub async fn run(&mut self, start_time_rx: Option<tokio::sync::oneshot::Receiver<Clock>>) {
        ...
        loop {
            ...
            let mut received =
                match timeout(Duration::from_secs(2), self.commands_receiver.recv()).await {
                ...
                .filter_map(|(index, sub)| {
                    match sub {
                        ...
                        Some(sub) => Some(SavedChannelState {
                            ...
                            local_channel_name: self.self_info.rx_channels[index]
                                .friendly_name
                                .read()
                                .unwrap()
                                .clone(),
    ...
    async fn load_state(&mut self) {
        let channels = {
            let result = self
                .state_storage
                .load::<SavedChannelsState>("rx_subscriptions")
                .or_else(|_| {
        ...
        for chst in channels {
            ...
            if lci >= self.channels.len() {
                warn!(
                    "loading state containing channel id {} while we have {} channels. State of this channel will be lost on next save.",
                    chst.local_channel_id,
                    self.channels.len()
                );
            ...
            self.subscribe(lci, &chst.tx_channel_name, &chst.tx_hostname)
                .await;
        }
    }
}
