...
impl<P: ProxyToSamplesBuffer> Channel<P> {
    fn report_lost_samples(&self, timestamp: Clock, num_samples: usize, reason: &str) {
        error!(
            "Lost {num_samples} samples at timestamp {timestamp} in channel id {} ({reason})",
            self.id
        );
    }
    fn read_samples_from_ringbuffer(
        &mut self,
...
impl<P: ProxyToSamplesBuffer> RealTimeSamplesReceiver<P> {
    ...
    pub fn get_available_num_samples(&mut self, start_timestamp: Clock) -> usize {
        self.get_min_max_end_timestamps()
            .map(|(end_ts, _)| {
                let diff = wrapped_diff(end_ts, start_timestamp);
                if diff > 0 { diff as Clock } else { 0 }
            })
            .unwrap_or(0)
            .try_into()
            .unwrap()
    ...
    pub fn get_samples(
        ...
        if let Some(ch) = chrecv.get_mut() {
            let start_timestamp = start_timestamp
                .wrapping_sub(ch.latency_samples)
                .wrapping_sub(buffer.len() as Clock);
...
fn get_min_max_end_timestamps<'a, P: ProxyToSamplesBuffer + 'a>(
    ...
    Some((
        ...
        clocks
            .iter()
            .max_by(|&&a, &&b| wrapped_diff(a, b).cmp(&0))?
            .to_owned()
            .try_into()
            .unwrap(),
    ))
...
impl<P: ProxyToSamplesBuffer> PeriodicSamplesCollector<P> {
    ...
    async fn run(&mut self) {
        ...
        let mut channels_buffers = (0..self.channels.len())
...
impl<P: ProxyToSamplesBuffer + Sync + Send + 'static> SamplesCollector<P> {
    ...
    pub fn new_realtime(
        ...
        RealTimeSamplesReceiver<P>,
    ) {
        let (tx, rx) = mpsc::channel(100);
        let (senders, receivers) = (0..self_info.rx_channels.len())
            .map(|chi| real_time_box_channel::channel(Box::new(None)))
            .unzip();
    ...
    pub async fn disconnect_channel(&self, channel_index: usize) {
        self.commands_sender
            .send(Command::DisconnectChannel { channel_index })
            .await
            .log_and_forget();
    }
