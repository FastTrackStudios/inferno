...
use super::flows_tx::{FPP_MAX_ADVERTISED, FlowInfo as TXFlowInfo};
...
pub async fn run_server(
    ...
    let server =
        UdpSocketWrapper::new(Some(self_info.ip_address), self_info.arc_port, shutdown).await;
        ...
        if request.opcode2().read() == 0 {
            match request.opcode1().read() {
                channels_and_flows_count::OPCODE => {
                    let total_channels_wtf =
                        self_info.tx_channels.len() + self_info.rx_channels.len(); // ??? not actually total number of channels but in some devices it is
                ...
                get_device_names::OPCODE => {
                    ...
                    let friendly_hostname_offset =
                        (bytes.get_wpos() + strings_offset).try_into().unwrap();
                    bytes.write_bytes(self_info.friendly_hostname.as_bytes());
                    bytes.write_u8(0);
                    let factory_hostname_offset =
                        (bytes.get_wpos() + strings_offset).try_into().unwrap();
                ...
                get_transmit_channels::OPCODE => {
                    // 0x2000 returns the user-visible TX channel name. Real Dante
                    // hardware returns the renamed name here (not just in 0x2010
                    // friendly-names), so we follow suit: prefer `friendly_name`
                    // when it's been set, fall back to `factory_name` otherwise.
                    // Without this, controllers that only query 0x2000 (e.g.
                    // netaudio's `channel list`) keep showing the factory label
                    // after a successful rename via 0x2013.
                    let mut common_descriptor_offset: u16 = 0;
                    paginate_respond(
                        &mut conn,
                        request.content(),
                        ...
                        |(channel_index, ch), bytes| {
                            ...
                            let friendly = ch.friendly_name.read().unwrap();
                            let name: &str = if friendly.is_empty() {
                                ch.factory_name.as_str()
                            } else {
                                friendly.as_str()
                            };
              ...
              Err(e) => {
                ...
                None
                    ...
                    if renamed_anything {
                        ...
                        conn.respond_with_code(0xFFFF /* TODO: really? */, &[])
                            .await;
                    }
                }
            ...
            match read_0term_str_from_buffer(content, name_offset) {
              ...
              Err(e) => {
                ...
                None
                    ...
                    mcast
                        .send(make_channel_change_notification(renamed_indices))
                        .await
                        .log_and_forget();
                    conn.respond_with_code(
                        if renamed_any {
                            1
                        } else {
                            0xFFFF /* TODO */
                    ...
                    let (code, response) = paginate_make_response(
                        ...
                        |(flow_index, flow_opt), bytes| -> Option<u16> {
                            flow_opt.as_ref().map(|flow_info| -> u16 {
                                ...
                                let local_tx_flow_name_offset =
                                    write_0term_str_to_bytebuffer(bytes, &flow_name);
                                let remote_hostname_offset = write_0term_str_or_0_to_bytebuffer(
                                    bytes,
                                    flow_info.rx_hostname.as_deref(),
                                );
                                let remote_rx_flow_name_offset = write_0term_str_or_0_to_bytebuffer(
                                    bytes,
                                    flow_info.rx_flow_name.as_deref(),
                                );
                                let is_multicast =
                                    remote_hostname_offset == 0 && remote_rx_flow_name_offset == 0;

                                align_wpos(bytes, 4);
                                let receiver_socket_descriptor_offset =
                                    bytes.get_wpos().try_into().unwrap();
                                bytes.write_bytes(
                                ...
                                for ch in &flow_info.local_channel_indices {
                                    bytes.write_u16(
                                        ch.map(|i| i + 1).unwrap_or(0).try_into().unwrap(),
                                    );
                                }

                                bytes.write_bytes(
                                    query_tx_flows::FlowDescriptorFooter {
              ...
              continue;
                                ...
                                flow_opt.as_ref().map(|flow_info| -> u16 {
                                    ...
                                    bytes.write_bytes(
                                        query_rx_flows::Descriptor2 {
                                            ...
                                            unknown3_800: 0x800,
                                            unknown4_0: 0,
                                            latency_ns: (flow_info.latency_samples as u64
                                                * 1_000_000_000u64
                                    ...
                                    let req_bits_in_mask = flow_info
                                        .channels_map
                                        .iter()
                                        .map(|bv| bv.len())
                                        .max()
                                    ...
                                    let bitmask_offsets = flow_info
                                        ...
                                        .map(|mask| {
                                            ...
                                            for _ in 0..words_per_bitmask {
                                                ...
                                                while single_bit != 0 {
                                                    word |= if mask.get(chi).unwrap_or(false) {
                                                        single_bit
                                                    } else {
                                                        0
                                                    };
                                    ...
                                    bytes.write_bytes(
                                        query_rx_flows::FlowDescriptorHeader {
                                            ...
                                            channels_count: flow_info
                                                ...
                                                .try_into()
                                                .unwrap(),
                                            words_per_bitmask: words_per_bitmask
                                                .try_into()
                                                .unwrap(),
                                            receiver_socket_descriptor_offset,
                                        }
                ...
                0x3300 => {
                    ...
                    conn.respond(&[0x38, 0x00, 0x38, 0xfd, 0x38, 0xfe, 0x38, 0xff])
                        .await;
                ...
                set_channels_subscriptions::OPCODE => {
                    ...
                    if let Some(channels_recv) = &subscriber {
                        ...
                        for req in deserialize_items::<
                            set_channels_subscriptions::SingleChannelSubscriptionRequest,
                        >(c_whole)
                        {
                            if req.local_channel_id == 0 {
                                continue;
                            }
                            let local_channel_index =
                                (req.local_channel_id - 1).try_into().unwrap();
                            if local_channel_index >= self_info.rx_channels.len() {
                                error!(
                                    "got connect/disconnect request for nonexisting channel {}",
                                    req.local_channel_id
                                );
                                continue;
                            }
                            if req.tx_channel_name_offset > 0 && req.tx_hostname_offset > 0 {
                                let str_or_none = |offset: u16| match offset {
                                    ...
                                    v => match read_0term_str_from_buffer(
                                        &c_whole,
                                        v as usize - HEADER_LENGTH,
                                    ) {
                                        Ok(s) => Some(s),
                                        Err(e) => {
                                ...
                                info!(
                                    "disconnect requested: local channel {}",
                                    req.local_channel_id
                                );
                ...
                x => {
                    error!(
                        "received unknown opcode1 {x:#04x}, content {}",
