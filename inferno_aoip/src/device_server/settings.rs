...
fn create_self_info(
    ...
    let process_id: u16 = settings
        .get("PROCESS_ID")
        .map(|s| s.parse().expect("PROCESS_ID must be u16"))
        .unwrap_or(0);
    ...
    settings
        .get("DEVICE_ID")
        .map(|idstr| {
            hex::decode_to_slice(idstr, &mut devid)
                .expect("invalid DEVICE_ID, should contain hex data");
        })
        .unwrap_or_else(|| {
    ...
    let short_app_name = if short_app_name.len() > 14 {
        &short_app_name[0..14]
    } else {
        short_app_name
    };
    ...
    if let Some(altport) = settings
        .get("ALT_PORT")
        .map(|s| s.parse().expect("ALT_PORT must be u16"))
    {
...
impl Settings {
    pub fn new(
        ...
        let tx_source_bit_depth = config
            ...
            .map(|s| {
                s.parse::<u8>()
                    .expect("invalid TX_SOURCE_BIT_DEPTH, must be one of: 16, 24, 32")
            })
            .unwrap_or(32);
        assert!(
