...
impl<P: ProxyToSamplesBuffer> FlowsReceiverInternal<P> {
            ...
            match sd.socket.recv_from(&mut buf) {
                Ok((recv_size, src)) => {
                    ...
                    if let Some(now) = clock.wrapping_now_in_timebase(sample_rate.into()) {
                        ...
                        sd.actual_latency_samples
                            .fetch_max(latency as _, Ordering::Relaxed);
                    }

                    if write {
                        let num_channels = sd.channels.len();
                        sd.last_packet_time
                            .store(ref_instant.elapsed().as_secs() as _, Ordering::Relaxed);
                            ...
                            if let Some(ch) = ch {
                                ...
                                ch.sinks.iter_mut().for_each(|sink| {
                                    ...
                                    match sd.bytes_per_sample {
                                        2 => sink
                                            .write_from_at(ts as usize, S16ReaderIterator(reader)),
                                        3 => sink
                                            .write_from_at(ts as usize, S24ReaderIterator(reader)),
                                        4 => sink
                                            .write_from_at(ts as usize, S32ReaderIterator(reader)),
                                        other => {
    ...
    fn run(&mut self, mut start_time_rx: Option<tokio::sync::oneshot::Receiver<Clock>>) {
        let keepalive_interval_between_flows =
            KEEPALIVE_INTERVAL / self.sockets.len().try_into().unwrap();
        ...
        let closing_samples_interval = self
            ...
            .map(|transfer| {
                Duration::from_nanos(
                    // TODO: /2 is a HACK
                    (transfer.max_interval_samples as u64 * 1_000_000_000u64
        ...
        loop {
            let write_to_rbs = self.sockets.iter().find(|opt| opt.is_some()).is_some()
                || self.silence_writers.len() > 0;
                    ...
                    if let Some(rx) = &mut start_time_rx {
                        ...
                        match rx.try_recv() {
                                            ...
                                            if let Some(channel) = channel_opt {
                                                channel.timestamp_shift = (0 as ClockDiff)
                                                    ...
                                                    .wrapping_add_unsigned(
                                                        channel.latency_samples.try_into().unwrap(),
                                                    )
                                                    as ClockDiff;
                                                // FIXME DRY
                                            }
                            ...
                            Err(tokio::sync::oneshot::error::TryRecvError::Closed) => {
                                panic!(
                                    "channel closed, unable to get start timestamp for ring buffer input"
                                );
                            }
                        }
                    }
            ...
            self.on_transfer
                .as_ref()
                .map(|transfer| (transfer.callback)());
            //} XXX
                    ...
                    Ok(command) => match command {
                        ...
                        Command::AddSocket { index, mut socket } => {
                            ...
                            self.poll
                                ...
                                .unwrap();
                            let previous =
                                std::mem::replace(&mut self.sockets[index], Some(socket));
                            debug_assert!(previous.is_none());
                        }
                        Command::RemoveSocket { index } => {
                            self.poll
                                .registry()
                                .deregister(&mut self.sockets[index].as_mut().unwrap().socket)
                                .unwrap();
                        ...
                        Command::ConnectChannel {
                            ...
                            let sink = if let Some(existing_index) = self
                                .silence_writers
                                .iter()
                                .position(|sw| Arc::ptr_eq(sw.sink.shared(), sink.shared()))
                            {
                                ...
                                if let Some(now) =
                                    self.clock.wrapping_now_in_timebase(self.sample_rate.into())
                                {
                                    sink.shared()
                                        .reset(now.wrapping_add_signed(timestamp_shift));
                                } else {
                                    error!(
                                        "clock not available, unable to reset ringbuffer when connecting channel"
                                    );
                                }
                                sink
                            };
                            ...
                            if channel_opt.is_none() {
                                let mut sinks = socket
                                    .empty_sinks_vecs
                                    .pop()
                                    .expect("ran out of empty sinks");
                        ...
                        Command::DisconnectChannel {
                            ...
                            if let Some(sd) = self.sockets[socket_index].as_mut() {
                                if let Some(ch) = sd.channels[channel_in_flow].as_mut() {
                                    let sink_index_opt = ch
                                        .sinks
                                        .iter()
                                        .position(|sink| Arc::ptr_eq(sink.shared(), &rb_shared));
                                    if let Some(sink_index) = sink_index_opt {
                                        let sink = ch.sinks.swap_remove(sink_index);
                                        if let Some(now) = self
                                            .clock
                                            .wrapping_now_in_timebase(self.sample_rate.into())
                                        {
                                            ...
                                            warn!(
                                                "no media clock, unable to initialize SilenceWriter"
                                            );
                                        }
                                    }
                                }
...
impl<P: ProxyToSamplesBuffer + Send + Sync + 'static> FlowsReceiver<P> {
    ...
    pub async fn add_socket(
        ...
        self.flows_info.write().unwrap()[local_index] = Some(FlowInfo {
            rx_port: port,
            channels_map: (0..channels_count)
                .map(|_| boolvec![false; self.max_channels])
                .collect(),
    ...
    pub async fn remove_socket(&self, local_index: usize) {
        ...
        self.commands_sender
            .send(Command::RemoveSocket { index: local_index })
            .await
            .log_and_forget();
        self.waker.wake().log_and_forget();
    }
    pub async fn connect_channel(
        ...
        debug!(
            "connecting channel: flow index={local_flow_index}, channel in flow: {channel_in_flow}"
        );
    ...
    pub async fn disconnect_channel(
        ...
        debug!(
            "disconnecting channel: flow index={local_flow_index}, channel in flow: {channel_in_flow}"
        );
        ...
        self.commands_sender
            .send(Command::DisconnectChannel {
                socket_index: local_flow_index,
                channel_in_flow,
                rb_shared,
            })
