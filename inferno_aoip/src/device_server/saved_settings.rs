...
impl SavedChannelsSettings {
    pub fn load(state_storage: Arc<StateStorage>, self_info: Arc<DeviceInfo>) -> Self {
        let mut r = Self {
            rx_channels: state_storage
                .load("rx_channels")
                .unwrap_or(SavedChannels { channels: vec![] }),
            tx_channels: state_storage
                .load("tx_channels")
                .unwrap_or(SavedChannels { channels: vec![] }),
            state_storage,
            self_info,
    ...
    pub fn rename_rx_channel(&mut self, index: usize, name: String) {
        Self::rename(
            &mut self.rx_channels.channels,
            &self.self_info.rx_channels,
            index,
            name,
        );
        self.state_storage
            .save("rx_channels", &self.rx_channels)
            .log_and_forget();
    }
    pub fn rename_tx_channel(&mut self, index: usize, name: String) {
        Self::rename(
            &mut self.tx_channels.channels,
            &self.self_info.tx_channels,
            index,
            name,
        );
        self.state_storage
            .save("tx_channels", &self.tx_channels)
            .log_and_forget();
    }
}
