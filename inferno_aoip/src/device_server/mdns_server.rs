...
use crate::{
    ...
    mdns_client::{MdnsClient, PointerToMulticast, self_origin_from_self_info},
    utils::LogAndForget,
};
...
impl DeviceMDNSResponder {
    ...
    pub fn add_tx_channel(&self, index: usize) {
        ...
        let service = |ch_name: &str, default: bool| {
            let name = Name::from_labels([
                format!("{}@{}", ch_name, self_info.friendly_hostname).as_bytes()
            ])
            .unwrap();
            let mut b = ServiceBuilder::new(
                service_type("_netaudio-chan"),
                name,
                self_info.flows_control_port,
            )
            .unwrap()
            .add_ip_address(IpAddr::V4(self_info.ip_address))
            ...
            .add_txt_truncated(format!(
                "pcm={} {:x}",
                self_info.bits_per_sample / 8,
                self_info.pcm_type
            ))
            .add_txt_truncated(kv("enc", self_info.bits_per_sample))
            .add_txt_truncated(kv("en", self_info.bits_per_sample))
            .add_txt_truncated(kv(
                "latency_ns",
                self_info.latency_ns, /* FIXME should be tx latency */
            ))
            .add_txt_truncated(format!("fpp={},{}", FPP_MAX_ADVERTISED, FPP_MIN))
            .add_txt_truncated(kv(
                "nchan",
                MAX_CHANNELS_IN_FLOW.min(self_info.tx_channels.len() as u16),
            ));
            if default {
        ...
        match handle.as_ref() {
            Some(handle) => {
                handle
                    .add_service(service(&txch.factory_name, true))
                    .log_and_forget();
                ...
                if txch.factory_name != *friendly_name {
                    handle
                        .add_service(service(&friendly_name, false))
                        .log_and_forget();
                }
            }
            None => {
                log::error!(
                    "BUG: trying to add channel using BroadcasterHandle after it was shut down"
                );
            }
        };
    }
    ...
    pub fn remove_tx_channel(&self, index: usize) {
        ...
        let remove = |ch_name: &str| {
            ...
            match self.handle.read().unwrap().as_ref() {
                Some(handle) => {
                    handle
                        .remove_named_service(service_type("_netaudio-chan"), name)
                        .log_and_forget();
                }
                None => {
                    log::error!(
                        "BUG: trying to remove channel using BroadcasterHandle after it was shut down"
                    );
                }
            }
        };
    ...
    pub fn add_multicast_bundle(
        ...
        let name =
            Name::from_labels(
                [format!("{}@{}", bundle_id, self_info.friendly_hostname).as_bytes()],
            )
            .unwrap();
        let handle = self.handle.read().unwrap();
        let service = ServiceBuilder::new(
            service_type("_netaudio-bund"),
            name,
            self_info.flows_control_port,
        )
        .unwrap()
        ...
        match handle.as_ref() {
            ...
            None => {
                log::error!(
                    "BUG: trying to add multicast bundle using BroadcasterHandle after it was shut down"
                );
            }
        }
    }
    ...
    pub fn remove_multicast_bundle(&self, bundle_id: usize) {
        ...
        match handle.as_ref() {
            Some(handle) => {
                handle
                    .remove_named_service(service_type("_netaudio-bund"), name)
                    .log_and_forget();
            }
            None => {
                log::error!(
    ...
    pub fn reserve_multicast_ip(&self, addr: Ipv4Addr) {
        ...
        match handle.as_ref() {
            ...
            None => {
                log::error!(
                    "BUG: trying to reserve multicast IP using BroadcasterHandle after it was shut down"
                );
            }
        }
    }
    pub fn remove_multicast_ip(&self, addr: Ipv4Addr) {
        ...
        match handle.as_ref() {
            Some(handle) => {
                handle
                    .remove_named_service(in_addr_type(), multicast_ip_to_name(addr))
                    .log_and_forget();
            }
            None => {
                log::error!(
