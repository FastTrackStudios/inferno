...
impl TransmitMulticasts {
            ...
            let mut flow_index = saved.flow_id - 1;
            {
                ...
                if flow_index >= bundles.len() || bundles[flow_index].is_some() {
                    if let Some(new_index) = bundles.iter().position(|b| b.is_none()) {
                        warn!(
                            "loading multicast flows: changing id {} to {}",
                            flow_index + 1,
                            new_index + 1
                        );
    ...
    pub async fn add_flow(&self, flow_index: usize, channel_indices: Vec<Option<usize>>) {
        self.add_flow_internal(flow_index, channel_indices, Ipv4Addr::UNSPECIFIED)
            .await
    }
    async fn add_flow_internal(
        ...
        let dst = {
            ...
            let (dst_addr, dst_port) = if let Some(tx) = flows_tx.as_mut() {
                ...
                error!(
                    "BUG: TransmitMulticasts::add_flow called but flows_tx is None. this flow will start working only after you restart Inferno"
                );
                (Ipv4Addr::UNSPECIFIED, 0)
            };
            bundles[flow_index] = Some(Bundle {
                local_channel_indices: channel_indices.clone(),
                dst_addr: dst_addr_arc.clone(),
            });
            (dst_addr, dst_port)
        };
        ...
        if !dst.0.is_unspecified() {
            ...
            tokio::spawn(async move {
                ...
                loop {
                    let mut ok = true;
                    ok &= !mdns_client
                        .is_multicast_ip_already_used(dst_addr)
                        .await
                        .unwrap_or(true);
                    if ok {
                    ...
                    }
                    {
                        ...
                        if let Some(flows_tx) = flows_tx_opt.as_mut() {
                            if !should_work.load(std::sync::atomic::Ordering::SeqCst) {
                                ...
                                warn!(
                                    "flows_tx will be destroyed soon, not activating transmitter"
                                );
                                break;
                            }
                            if ok {
                                ...
                                dst_addr_arc
                                    .store(dst_addr.to_bits(), std::sync::atomic::Ordering::SeqCst);
                                {
                                    let mut mcasts = multicasts_by_channel.write().unwrap();
                                    for (channel_in_bundle, chi_opt) in
                                        channel_indices.iter().enumerate()
                                    {
                                ...
                                warn!(
                                    "multicast address conflict detected: {dst_addr:?}, retrying"
                                );
                                flows_tx
                                    .remove_multicast_flow(flow_index.try_into().unwrap())
                                    .await
                                    .log_and_forget();
                            ...
                            error!(
                                "trying to activate multicast flow but we have no flows transmitter active"
                            );
                            break;
                        }
                    }
        ...
        if let Some(bundle) = bundles[flow_index].take() {
            ...
            if let Some(flows_tx) = flows_tx_opt.as_mut() {
                ...
                if addr_int != 0 {
                    ...
                    self.mdns_server
                        .remove_multicast_ip(Ipv4Addr::from_bits(addr_int));
                }
                let result = flows_tx
                    .remove_multicast_flow(flow_index.try_into().unwrap())
                    .await;
                {
                    ...
                    for (_channel_in_bundle, chi_opt) in
                        bundle.local_channel_indices.iter().enumerate()
                    {
                        if let Some(chi) = chi_opt {
                            mcasts.remove(chi);
                        }
    ...
    pub async fn shutdown(&self) {
        ...
        for i in 0..MAX_FLOWS {
            self.remove_flow_internal(i.try_into().unwrap(), true)
                .await
                .log_and_forget();
        }
    }
            ...
            flows: self
                ...
                .filter_map(|(flow_index, bundle_opt)| {
                    bundle_opt.as_ref().map(|bundle| SavedMulticastTXFlow {
                        ...
                        dst_addr: Ipv4Addr::from_bits(
                            bundle.dst_addr.load(std::sync::atomic::Ordering::SeqCst),
                        ),
                        dst_port: MEDIA_PORT,
