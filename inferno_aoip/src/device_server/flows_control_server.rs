...
use super::flows_tx::{FPP_MAX, FlowsTransmitter, MAX_FLOWS};
...
pub async fn run_server(
    ...
    let server = UdpSocketWrapper::new(
        Some(self_info.ip_address),
        ...
        if request.opcode2().read() == 0 {
            match request.opcode1().read() {
                0x0100 => {
                    ...
                    let hostname = read_0term_str_from_buffer(req_bytes, hostname_offset)
                        .unwrap()
                        .to_owned();
                    let rx_flow_name = read_0term_str_from_buffer(req_bytes, rx_flow_name_offset)
                        .unwrap()
                        .to_owned();
                    ...
                    if req_bytes[remote_descr_offset] != 0x08
                        || req_bytes[remote_descr_offset + 1] != 0x02
                    {
                        warn!(
                    ...
                    let rx_port = make_u16(
                        req_bytes[remote_descr_offset + 2],
                        req_bytes[remote_descr_offset + 3],
                    );
                    let ip_bytes: [u8; 4] = req_bytes
                        [remote_descr_offset + 4..remote_descr_offset + 8]
                        .try_into()
                        .unwrap();
                    let rx_ip = Ipv4Addr::from(ip_bytes);

                    info!(
                        "{hostname} requesting flow {rx_flow_name} of channel indices {channel_indices:?} at {sample_rate}Hz {bits_per_sample}bit {fpp} fpp to {rx_ip}:{rx_port}"
                    );
                    if channel_indices
                        .iter()
                        .flatten()
                        .find(|&&chi| chi >= self_info.tx_channels.len())
                        .is_some()
                    {
                    ...
                    if sample_rate != self_info.sample_rate {
                        ...
                        conn.respond_with_code(FlowControlError::SampleRateMismatch as u16, &[])
                            .await;
                        continue;
                    }
                    if fpp > FPP_MAX {
                    ...
                    match result {
                        ...
                        Err(e) => {
                            ...
                            conn.respond_with_code(FlowControlError::TooManyTXFlows as u16, &[])
                                .await;
                        }
                    }
                }
                0x0101 => {
                        ...
                        .await
                    {
                        ...
                        conn.respond_with_code(FlowControlError::FlowNotFound as u16, &[])
                            .await;
                    }
                }
                0x0102 => {
                    ...
                    if let Ok(_flow_index) = flows_tx
                        .lock()
                        .await
                        .as_mut()
                        .unwrap()
                        .set_channels(handle, channel_indices.clone())
                        .await
                    {
