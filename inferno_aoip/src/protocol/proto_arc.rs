...
pub mod channels_and_flows_count {
    use binary_serde::{BinarySerde, BitfieldBitOrder, binary_serde_bitfield};
    ...
    OutItem: BinarySerde,
{
    let start_index = match extract_start_index(request_payload) {
        ...
        None => {
            error!(
                "unable to extract start index from request payload {}",
                hex::encode(request_payload)
            );
            return (0xFFFF /* TODO */, vec![]);
        }
    };
    let (have_more, bytes) =
        serialize_items(space_items, source.into_iter().skip(start_index), transform);
...
{
    let (code, bytes) =
        paginate_make_response(connection, request_payload, space_items, source, transform);
...
impl<'a, T: BinarySerde> Iterator for ItemsInPacketIterator<'a, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            match T::binary_deserialize(
                &self.items_bytes[item_start..item_end],
...
mod tests {
    ...
    fn flags2_bitfield_roundtrip() {
        ...
        assert_eq!(
            original.supports_tx_channel_rename,
            deserialized.supports_tx_channel_rename
        );
        assert_eq!(
            original.supports_tx_multicast,
            deserialized.supports_tx_multicast
        );
    ...
    fn destination_socket_descriptor_roundtrip() {
        ...
        let bytes = original.binary_serialize_to_array(binary_serde::Endianness::Big);
        let deserialized = DestinationSocketDescriptor::binary_deserialize(
            bytes.as_slice(),
            binary_serde::Endianness::Big,
        )
        .unwrap();
    ...
    fn get_receive_channels_channel_descriptor_roundtrip() {
        ...
        assert_eq!(
            ...
            deserialized.subscription_status
        );
    }
    ...
    fn query_rx_flows_descriptor2_roundtrip() {
        ...
        let bytes = original.binary_serialize_to_array(binary_serde::Endianness::Big);
        let deserialized = query_rx_flows::Descriptor2::binary_deserialize(
            bytes.as_slice(),
            binary_serde::Endianness::Big,
        )
        .unwrap();
    ...
    fn deserialize_items_empty() {
        let items: Vec<get_receive_channels::ChannelDescriptor> =
            deserialize_items(&[0, 0]).collect();
