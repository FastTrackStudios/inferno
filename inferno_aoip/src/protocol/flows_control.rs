...
use super::req_resp::{HEADER_LENGTH, make_packet, req_resp_packet};
...
impl FlowsControlClient {
    pub fn new(self_info: Arc<DeviceInfo>) -> Self {
        Self {
            seqnum: AtomicU16::new(1),
            self_info,
        }
    }
    async fn connect(&self, rem_addr: &SocketAddr) -> tokio::io::Result<UdpSocket> {
        let socket =
            UdpSocket::bind(SocketAddr::new(IpAddr::V4(self.self_info.ip_address), 0)).await?;
    ...
    pub async fn request_flow(
        ...
        let resp = self
            .send_and_wait_for_reply(&mut recvbuf, &socket, dbcp1, 0x0100, body.as_bytes())
            .await?;
